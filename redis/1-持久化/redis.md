...
（为什么会fork子进程？redis是单线程，不是说整个redis是单线程，是说redis处理网络、客户端的命令是单线程的，没有并发问题，比如三个客户端同时达到redis并写入数据，由于接收网络是单线程，会将三条命令串行化，排队，依次执行。

因为redis是单线程的，如果用

save命令会使用主进程，而不是fork子进程）
...
#### 【为什么会出现aof，就是因为rdb的缺点，一旦删除持久化文件，数据就没有了，rdb文件话你了，可以通过命令修复。rdb数据丢失太多，所以出现aof】
...
也是根据配置文件的dir配置，和rdb持久化一样。（appendonly yes启动aof持久化；appendfsync everysec配置不需要更改）
...
aof（rdb会fork子进程，aof持久化是1秒一次，aof只有重写才会fork子进程；aof重写是给aof文件瘦身的）
